...
function addMessage(payload) {
  return function (dispatch, getState) {
    ...
    var msgId = _core.utilities.uuid();
    ...
    return new Promise(function (resolve, reject) {
      ...
      setTimeout(function () {
        return dispatch({
          types: [_core.ActionTypes.REQUEST_SEARCH, _core.ActionTypes.REQUEST_SEARCH_SUCCESS, _core.ActionTypes.REQUEST_SEARCH_FAILURE],
          query: query,
          context: context,
          api: 'search',
          payload: payload
          ...
          if ((0, _utils.checkIfAwaitingResponse)(response) && searchInput !== 'voice') {
            ...
            dispatch(_core.Actions.Search.clearQueryTerm());
...
function loadMore(message) {
  var _message$search = message.search,
      q = _message$search.q,
      facet_query = _message$search.facet_query,
      searchAdapterOptions = _message$search.searchAdapterOptions;

  return function (dispatch, getState) {
    var currentPage = getState().QueryState.page;
    dispatch(_core.Actions.Search.changePage(++currentPage));
    dispatch(_core.Actions.Search.executeSearch({
      routeChange: false,
      appendResult: true,
      extraParams: {
        /* Additional params */
